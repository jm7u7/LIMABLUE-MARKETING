...
function LoginGate() {
  ...
  if (!session && !pending) return <LoginScreen onLogin={setPending} />;
  if (pending) return <RoleTransition cargo={cargoMeta(pending.cargo)} username={pending.username} onFinish={() => { setSession(pending); setPending(null); }} />;
  if (session.cargo === "visitante") return <VisitanteDashboard onLogout={() => setSession(null)} />;
